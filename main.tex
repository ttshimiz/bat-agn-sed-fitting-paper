...
Overall all three models agree on $f_{\rm AGN}$ within a scatter of 0.15. The scatter around a 1--1 correlation (shown by the dashed black line) between each model is 0.15, 0.13, and 0.10 for C12 vs. DecompIR, C12 vs. D14, and DecompIR vs. D14 respectively. Because this is comparable to the statistical uncertainty we determined through the individual model-fitting, we add this in quadrature to the uncertainties in Tables~\ref{tab:c12_params},~\ref{tab:decompir_params},~and~\ref{tab:d14_params}.

The 15 per cent uncertainty between the three models represents our general lack of knowledge about the details of decomposing broadband SEDs of AGN host galaxies. The estimated $f_{\rm AGN}$ is highly dependent on the assumed models of both the host galaxy and AGN, and currently at best we can only constrain to within 15 per cent. This influences then the calculations of both the SFR (based on $L_{\rm SF}$) and the IR portion of AGN luminosity ($L_{\rm AGN, IR}$). 

%%%%%%%%%%%%%%%%%%%%%%%% FIGURE: AGN FRACTION COMPARISON %%%%%%%%%%%
\begin{figure*}
\includegraphics[width=\textwidth]{figures/agn_frac-comparison}
\caption{Correlations and histograms of $f_{\rm AGN}$ from the three SED models.  A color version of this figure is available in the online publication. \label{fig:agn_frac_comp}}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
